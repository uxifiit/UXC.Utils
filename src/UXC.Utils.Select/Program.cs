...
using UXC.Utils.Select.Configuration;
using UXC.Utils.Select.Observers;
using UXI.Filters;
...
namespace UXC.Utils.Select
{
    class Program
    {
        ...
        {
            return new SingleFilterHost<SelectContext, SelectOptions>
            (
                // filter:
                new RelayFilter<TimestampedDataPayload, TimestampedDataPayload, SelectOptions, SelectContext>((source, _, context) => Select(source, context)),

                // configurations:
                new TimestampedDataPayloadSerializationConfiguration(),
                new SelectionSerializationConfiguration(),
                new RelayFilterConfiguration<SelectContext, SelectOptions>(Configure)
            ).Execute(args);
        }


        private static IObservable<TimestampedDataPayload> Select(IObservable<TimestampedDataPayload> source, SelectContext context)
        {
            var result = source;
